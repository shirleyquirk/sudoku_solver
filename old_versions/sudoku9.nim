...
var tries = 0
proc ratings(g:Sudoku):seq[float] =
  ...
  collect(for e in 0..80:#e in g.empties:
    let x = e mod 9
    let y = e div 9
    if e in g.empties: (rows[y] + cols[x] + squares[x div 3 + 3 * (y div 3)]).float - 19.5
    else: 0.0
    )
...
for k,v in sudokuData:
  ...
  parameters.add(k.toSudoku.ratings)#hmm.
...
echo params.dim
#params expectation
let param_expect = zeros(81)#vector(collect(for r in params.columns: r.foldl(a+b) / r.len.float))
echo "parameter expected value:", param_expect
...
var W = Cxy * Cyy#echo Cxy.dim,',',Cyy.dim
#echo W
import math
W = matrix(@[collect(for i in 0..80:
  -tanh((i.float - 40)*0.5))])# 10.0 - 20.0*(i.float / 80))])
