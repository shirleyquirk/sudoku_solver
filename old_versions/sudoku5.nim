...
type
  ...
  Sudoku = object
    ...
    cols: array[9,int]
    squares: array[9,int]
    emptyqueue: seq[tuple[rating:int,idx:int]]#Deque[int]
...
proc empty(g:var Sudoku, pos:int){.inline.} =
  g.g[pos] = '.'
  g.emptyqueue.add @[(rating:ord('?')-ord('a'),idx:pos)]# & g.emptyqueue
  sort(g.emptyqueue)
  #g.emptyqueue.shuffle
  #assert isSorted(g.emptyqueue),$g.emptyqueue
  #sort(g.emptyqueue)
  #g.empties.incl pos
import strformat
...
proc resolv(g:var Sudoku):bool =# string =
    inc tries
      ...
      #echo g
      #echo g.emptyqueue
    if g.emptyqueue.len > 0:
        let i = g.emptyqueue.pop.idx
